...
const parseCategoryData = (cityAdata, otherCityData) => (
  category,
  catIndex
) => {
  return {
    ...
    data: category.data
      ...
      .map((data, dataIndex) => {
        const otherCityDataFiltered = otherCityData.categories.filter(
          category => !!cityAdata.categories.find(c => c.id === category.id)
        );
        const catInOtherCity = otherCityDataFiltered.find(
          c => c.id === category.id
        ) || { data: [] };
        const cityAcategories = cityAdata.categories.map(z => z.data);
        const filterdataInOtherCity = catInOtherCity.data.filter(
          d => d.id === cityAcategories.find(d => d.id === data.id)
        );
        const dataInOtherCity = catInOtherCity.data.find(
          d => d.id === data.id
        ...
        return {
          ...data,
          difference: dataInOtherCity
            ? data[valueType] - dataInOtherCity[valueType]
            : 0
        };
      })
  };
...
export default class CompareCities extends React.Component {
  ...
  toObject = arr => {
    ...
    for (var i = 0; i < arr.length; i++) {
      new_array.push({
        ...
        value: arr[i].float_value
          ? parseFloat(arr[i].float_value).toFixed(2)
          : arr[i].int_value
          ? parseInt(arr[i].int_value)
          : arr[i].percent_value
          ? parseFloat(arr[i].percent_value).toFixed(3)
          : arr[i].string_value
          ? arr[i].string_value
          : arr[i].currency_dollar_value
      });
    }
    return new_array;
  };
  ...
  getArrays = (city, index) => {
    return city[index] ? city[index].data : null;
  };

  getPositiveScores = (city, index) => {
    return this.getArrays(city, index)
      ? this.getArrays(city, index).filter(object => object.difference > 0)
      : null;
  };

  getPositiveObjects = (city, index) => {
    return this.getPositiveScores(city, index)
      ? this.toObject(this.getPositiveScores(city, index))
      : null;
  };
  ...
  getNegativeObjects = (city, index) => {
    return this.getNegativeScores(city, index)
      ? this.toObject(this.getNegativeScores(city, index))
      : null;
  };
  ...
  renderLabel = (dataId, city) => {
    const cityKey = city === "a" ? "cityADetails" : "cityBDetails";
    const category = this.state[cityKey].find(c => c.id === dataId);
    const categB = this.state["cityBDetails"].find(c => c.id === dataId);
    return category
      ? category.data.map((data, index) => (
          <Table.Row>
            <Table.Cell>{data.label}</Table.Cell>
            {this.renderData(data)}
            {categB && categB.data.find(b => b.id === data.id) ? (
              this.renderData(categB.data.find(b => b.id === data.id))
            ) : (
    ...
    return (
      <Table.Cell>
        {data.float_value
          ? data.float_value
          : data.percent_value
          ? parseFloat(data.percent_value).toFixed(2)
          : data.string_value
          ? data.string_value
          : data.currency_dollar_value
          ? data.currency_dollar_value
          : data.int_value}{" "}
        ...
        {data.difference ? parseFloat(data.difference).toFixed(2) : ""}
      </Table.Cell>
    );
  };
      ...
      <React.Fragment>
        ...
        <div class="compared-cities-wrapper">
          <div className="card" style={{ borderTop: "solid #29b6f6 4px" }}>
            <div className="ui white circular label">
              {parseFloat(this.state.cityAranking).toFixed(1)}
            </div>
            <img
              src={this.state.cityAUrl}
              alt={this.props.location.state.cities[0]}
              style={{ width: "100%" }}
            />
            <div className="container">
              <h2 className="city">
                <b>{this.props.location.state.cities[0]}</b>
              </h2>
          ...
          <div className="card" style={{ borderTop: "solid #e57373 4px" }}>
            <div class="ui white circular label">
              {parseFloat(this.state.cityBranking).toFixed(1)}
            </div>
            <img
              src={this.state.cityBUrl}
              alt={this.props.location.state.cities[1]}
              style={{ width: "100%" }}
            />
            <div class="container">
              <h2 className="city">
                <b>{this.props.location.state.cities[1]}</b>
              </h2>
        ...
        <div className="radar-wrapper">
          <div>
            {this.state.chartAdata[0] && this.state.chartBdata[0] ? (
              <Rad
                cityA={this.props.location.state.cities[0]}
                cityB={this.props.location.state.cities[1]}
                dataA={this.state.chartAdata}
                dataB={this.state.chartBdata}
              />
            ) : null}
          </div>
          <div className="right-div-salary">
            <h3 className="how-compares">
              How does {this.props.location.state.cities[0]} compare to{" "}
              {this.props.location.state.cities[1]}?
            </h3>
            <Tab
              getPositiveObjects={this.getPositiveObjects}
              getNegativeObjects={this.getNegativeObjects}
              cityAdetails={this.state.cityADetails}
              cityBdetails={this.state.cityBDetails}
              cityA={this.props.location.state.cities[0]}
              cityB={this.props.location.state.cities[1]}
            />
          </div>
        ...
        <div className="compared-cities-wrapper">
          <div className="jobs">
            ...
            <JobSalariesChart
              cityA={this.props.location.state.cities[0]}
              cityB={this.props.location.state.cities[1]}
              salaryA={renderSalaries(0, parsedAverageSalaryCityA)}
              salaryB={renderSalaries(1, parsedAverageSalaryCityB)}
            />
          </div>
          <div className="jobs">
            {this.state.selectedJob === "Compare Job Salaries" ? null : (
              <div>
                <h3 className="average-salary">
                  {this.state.selectedJob} (USD)
                </h3>
                <JobSalariesChart
                  cityA={this.props.location.state.cities[0]}
                  cityB={this.props.location.state.cities[1]}
                  salaryA={renderJobSalaries(0, parsedJobSalaryCityA)}
                  salaryB={renderJobSalaries(1, parsedJobSalaryCityB)}
                />
              </div>
            )}
          </div>
